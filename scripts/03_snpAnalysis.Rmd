...
```{r finalPCA, echo = FALSE, results='hide', fig.cap="*PCA for all samples including the outgroup and indicating the sample collection region for the 2012 samples.*"}
fs <- 14
ps <- 3
...
leftN <- data_frame(x = c(138.7595, 138.76, 138.76),
                    y = c(-31.234, -31.2335, -31.2325))
rightN <- data_frame(x = c(138.76, 138.76, 138.7605),
...
bind_rows(
  ...
  mutate(Status = factor(Status, levels = c("Pre", "Post")),
         Central = sampleID %in% filter(pcaForPlot, grepl("Central", Population))$sampleID,
...
```{r runFLK, results='hide', eval=FALSE}
...
pdf(file.path("..", "figures", "Figure3.pdf"), height = 0.6*h, width = w, onefile = FALSE)
sigSNPs %>% 
  dplyr::select(-snpID) %>% 
  lapply(which) %>% 
  fromList() %>% 
...
knitr::include_graphics(file.path("..", "figures", "Figure3.png"))
```
...
```{r genotypesBothModels, echo = FALSE,fig.cap = "Estimated genotype frequencies for SNPs found to be significant under both Full Genotype and FLK models. In each case the `A` allele represents the major allele in the 1996 population.", fig.width=9, fig.height=6}
sigSNPs %>%
  filter(genotype, flk) %>%
  dplyr::select(snpID) %>%
  left_join(allData) %>%
...
```{r flkModelOnly, echo = FALSE,fig.cap = "Estimated allele frequencies for SNPs found to be significant under analysis by FLK only. In each case the `A` allele represents the major allele in the 1996 population.", fig.width=9, fig.height=9}
sigSNPs %>%
  filter(!genotype, flk) %>%
  dplyr::select(snpID) %>%
  left_join(allData) %>%
