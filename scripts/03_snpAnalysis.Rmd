...
ggMap <- get_map(loc, zoom = 12, maptype = "terrain", source = "google")
zoomPlot <- ggmap(ggMap, extent = "normal", maprange = FALSE) +
  ...
  scale_colour_manual(values = c(rgb(0, 0, 0, 0.7),
                                 ...
                                 rgb(0.2, 0.8, 0.2, 0.9),
                                 rgb(0.5, 0.5, 1, 0.7))) +
  coord_map(xlim=c(attr(ggMap, "bb")$ll.lon, attr(ggMap, "bb")$ur.lon),
            ylim=c(-31.33, -31.18)) +
  guides(colour = FALSE) +
  labs(x = "Longitude",
       y = "Latitude") +
...
```{r plotWithInset, echo = FALSE, fig.cap="Collection points for all 2012 samples with colours showing sub-populations defined by PCA analysis.", fig.height=7, fig.width=8}
grid.newpage()
...
get_map(zoomLoc, zoom = 15, maptype = "terrain", source = "google") %>%
  ggmap() +
  # geom_point(data= pcaForPlot, aes(Longitude, Latitude, colour = Population),
  #            size = 3) +
  geom_jitter(data= pcaForPlot, aes(Longitude, Latitude, colour = Population), size = 3, width = 0.0003, height = 0) +
...
```{r upSetSNPs, echo = FALSE, fig.cap="Overlap between the lists of SNPs considered as associated with the different populations under either of the two analytic approaches, using an FDR of 0.1 for 'Analysis by Genotype' and Bayescan, with an FDR of 0.05 for 'Analysis by Allele' and FLK.", fig.width=6, fig.height=5, results='hide', fig.show='asis'}
sigSNPs %>% 
  dplyr::select(-snpID) %>% 
  lapply(which) %>% 
  fromList() %>% 
