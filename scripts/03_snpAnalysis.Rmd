...
pcaForPlot <- outgroupPCA$x %>%
  ...
  mutate(Central = `GPS waypoint` %in% central,
         ...
         PCATail = grepl("2012", Population) && PC3 > 3 && PC2 < 0) 
```
...
p13 <- ggplot(pcaForPlot, aes(PC1, PC3, colour = Population)) +
  ...
  guides(colour = FALSE) +
  theme(text = element_text(size = 12))
p23 <- ggplot(pcaForPlot, aes(PC2, PC3, colour = PCATail)) +
  geom_point(size = 3) +
  scale_colour_manual(values = c(rgb(0, 0, 0, 0.7),
                                 rgb(0.8, 0.2, 0.8, 0.7),
                                 rgb(0.2, 0.2, 0.8, 0.7),
...
ggmap(ggMap) +
  geom_point(data= pcaForPlot, aes(Longitude, Latitude, colour = PCATail))
```

```{r}
zoomLoc <- c(138.753, -31.242)
get_map(zoomLoc, zoom = 16, maptype = "hybrid", source = "google") %>%
  ggmap() +
  geom_point(data= pcaForPlot, aes(Longitude, Latitude, colour = PCATail),
             size = 3)
```
...
Under H<sub>0</sub>, the number expected using &#945; = 0.05 would be `r floor(0.05*ncol(minorAlleleCounts))`, and as this number was nearly double that expected, this was taken as evidence of this being a genuine point of concerning this data.
...
```{r vennSNPs, echo = FALSE, fig.cap="Overlap between the lists of SNPs considered as associated with the different populations under either of the two analytic approaches, using an FDR of 0.1.", fig.width=6, fig.height=5, results='hide', fig.show='asis'}
grid.newpage()
draw.pairwise.venn(
  area1 = sum(sigSNPs$genotype),
  area2 = sum(sigSNPs$allele),
  cross.area = nrow(filter(sigSNPs, genotype, allele)),
...
sigSNPs %>%
  ...
  dplyr::select(snpID, Chr, BP, `Change in log(OR)`, starts_with("P"), Genotype_p, Allele_p = p) %>%
  pander(justify = "llrrrrrr",
         ...
         caption = paste("Summary of changes in the major (P) allele between the two timepoints.",
                         ...
                         "Results from testing by genotype or allele are given as raw p-values.",
                         "All SNPs were considered as differentially associated with the two populations under both analyses to an FDR of 10\\%."))
```
...
sigSNPs %>%
  ...
  arrange(Chr, BP) %>%
  left_join(alleleResults) %>%
  dplyr::select(Chr, BP, snpID, `Change in log(OR)`, P_1996 = `1996`, P_2012 = `2012`, Allele_p = p) %>%
  pander(justify = "llrrrrr",
         ...
         caption = paste("Summary of changes in the major (P) allele between the two timepoints.",
                         ...
                         "All SNPs were considered as differentially associated with the two populations under the allele count analysis to an FDR of 10\\%."))
```

```{r alleleModelOnly, echo = FALSE,fig.cap = "Estimated allele frequencies for SNPs found to be significant under analysis by allele only. In each case the `A` allele represents the major allele in the 1996 population.", fig.width=9, fig.height=7}
sigSNPs %>%
  filter(!genotype, allele) %>%
  dplyr::select(snpID) %>%
  left_join(allData) %>%
  ...
  scale_fill_manual(values = c(rgb(0.1, 0.1, 0.1), rgb(0.2, 0.2, 0.8))) +
  facet_wrap(~snpID) +
  theme(legend.position = c(7/8, 1/8))
```
...
sigSNPs %>%
  ...
  pander(justify = "llrrrrr",
         ...
         caption = paste("Summary of changes in heterozygosity between the two timepoints.",
                         ...
                         "However, no significant changes in allele frequencies were detected using an FDR of 10\\% for the analysis of allele counts.",
...
sigSNPs %>%
  ...
  scale_fill_manual(values = c(rgb(0.1, 0.1, 0.1), rgb(0.2, 0.2, 0.8))) +
  facet_wrap(~snpID) +
  theme(legend.position = c(0.5, 1/8),
