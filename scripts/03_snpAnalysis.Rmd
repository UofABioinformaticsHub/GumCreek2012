...
p23 <- ggplot(pcaForPlot, aes(PC2, PC3, colour = Population)) +
  ...
  theme(text = element_text(size = fs),
        ...
        plot.margin = unit(c(1, 1, 1, 1)*1.5, "mm"),
        legend.position = "none")
vp12 <- viewport(0, 0, 0.735, 0.5, just = c(0, 0))
...
saPoly <-  file.path("..", "60804_shp", "Gazeteer") %>%
  readOGR(layer = "GazetteerAreas") 
proj4string(saPoly) <- proj4string(gc)
```
...
zoomPlot <- ggmap(ggMap, extent = "normal", maprange = FALSE) +
  geom_text(x = 138.74, y = -31.29, 
            aes(label = "Flinders Ranges NP"), colour = "grey10") +
  geom_text(aes(x, y, label = "Gum Creek Homestead"),
            data = as.data.frame(gc), nudge_y = 0.005) +
  geom_point(aes(x, y), data = as.data.frame(gc)) +
  ...
  geom_polygon(data = subset(saPoly, F_CODE == "PARK"),
               aes(long, lat, group = group),
               fill = rgb(1, 1, 1, 0), colour = "grey10", size = 0.1) +
  geom_point(data= pcaForPlot, aes(Longitude, Latitude, colour = Population), size = ps) +
  scale_colour_manual(values = popCols) +
  coord_map(xlim=c(138.618, 138.81),
            ylim=c(-31.335, -31.18)) +
  guides(colour = FALSE) +
  labs(x = "Longitude",
       y = "Latitude") +
...
vp2 <- viewport(0.315, 0.3, 0.33, 0.33)
...
reducedPCA$x %>%
  ...
  mutate(Central = sampleID %in% filter(pcaForPlot, grepl("Central", Population))$sampleID,
         ...
         Population = if_else(grepl("[Oo]ra", Population), 
                              if_else(Central, "2012 (Central)", "2012 (Outer)" ), c(gc = "1996", tf = "2012")[Population])) %>%
