...
sumData <- file.path("..", "data", "batch_3.sumstats.tsv.gz") %>%
  ...
  mutate(Chr = gsub("^gi\\|.+\\|ref\\|(.+)\\|$", "\\1", Chr),
         Chr = as.character(ncbi2Ens[Chr, "Chr"]),
...
snp2GR <- sumData %>%
  ...
  makeGRangesFromDataFrame(seqinfo = seqinfo(ensGenes),
...
- This marked 17,174 SNPs as containing duplicated information
...
- This gave 28,922 candidate SNPs for downstream analysis.
...
This gave the **final total of 18,728 SNPs** for downstream analysis.
