...
The set of `r nrow(testData)/2` SNPs previously obtained for testing after all filtering steps were then tested using the FLK model.
...
```{r plotFlkSnps, echo=FALSE, fig.cap="Comparison of allele frequencies in both populations for all tested SNPs. SNPs considered as significant under FLK are highlighted in red.", fig.width=8, fig.height=6}
flkResults %>%
  mutate(Sig = FDR < 0.05) %>%
